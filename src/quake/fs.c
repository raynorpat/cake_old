...
cvar_t scr_screenshot_name = {"scr_screenshot_name", "q"};
cvar_t fs_empty_files_in_pack_mark_deletions = {"fs_empty_files_in_pack_mark_deletions", "0"};
...
# endif
	...
	{
		Com_Printf ("Compressed files support disabled\n");
		return false;
	}

	Com_Printf ("Compressed files support enabled\n");
	return true;
}
...
====================
*/
qbool PK3_GetEndOfCentralDir (const char *packfile, int packhandle, pk3_endOfCentralDir_t *eocd)
{
...
int PK3_BuildFileList (pack_t *pack, const pk3_endOfCentralDir_t *eocd)
{
	...
	central_dir = (unsigned char *)malloc (eocd->cdir_size);
...
pack_t *FS_LoadPackPK3 (const char *packfile)
{
	...
	{
		Com_Printf ("%s is not a PK3 file\n", packfile);
		close(packhandle);
		return NULL;
	}
	...
	{
		Com_Printf ("%s is a multi-volume ZIP archive\n", packfile);
		close(packhandle);
		return NULL;
	}
	...
	{
		Com_Printf ("%s contains too many files (%hu)\n", packfile, eocd.nbentries);
		close(packhandle);
		return NULL;
	}
#endif
	...
	pack->files = (packfile_t *)malloc(eocd.nbentries * sizeof(packfile_t));
	memset(pack->files, 0, sizeof(*pack->files));
...
====================
*/
qbool PK3_GetTrueFileOffset (packfile_t *pfile, pack_t *pack)
{
	...
	{
		Com_Printf ("Can't retrieve file %s in package %s\n", pfile->name, pack->filename);
		return false;
	}
									 ...
									 fs_offset_t realsize, int flags)
{
	...
	while (left <= right)
	{
		...
		if (!diff)
			Com_Printf ("Package %s contains the file %s several times\n", pack->filename, name);
...
pack_t *FS_LoadPackPAK (const char *packfile)
{
	...
	{
		Com_Printf ("%s has an invalid directory size\n", packfile);
		close(packhandle);
		return NULL;
	}
	...
	{
		Com_Printf("%s is an incomplete PAK, not loading\n", packfile);
		free(info);
		close(packhandle);
		return NULL;
	}
	...
	pack->handle = packhandle;
	pack->numfiles = 0;
	pack->files = (packfile_t *)malloc(numpackfiles * sizeof(packfile_t));
	memset(pack->files, 0, sizeof(*pack->files));
...
================
*/
static qbool FS_AddPack_Fullpath(const char *pakfile, qbool *already_loaded, qbool keep_plain_dirs)
{
	searchpath_t *search;
	pack_t *pak = NULL;
	...
	if(!strcasecmp(ext, "pak"))
		...
		Com_Printf("\"%s\" does not have a pack extension\n", pakfile);
	...
	{
		Com_Printf("unable to load pak \"%s\"\n", pakfile);
		return false;
	}
}
...
qbool FS_AddPack(const char *pakfile, qbool *already_loaded, qbool keep_plain_dirs)
{
	...
	{
		Com_Printf("could not find pak \"%s\"\n", pakfile);
		return false;
	}

	Q_snprintf(fullpath, sizeof(fullpath), "%s%s", search->filename, pakfile);
...
void FS_AddGameDirectory (const char *dir)
{
	...
	for (i = 0;i < list.numstrings;i++)
	{
		...
		{
			Q_snprintf (pakfile, sizeof (pakfile), "%s%s", dir, list.strings[i]);
...
void FS_Rescan (void)
{
	...
	if (strcmp(com_modname, gamedirname1))
		Cvar_Set (&scr_screenshot_name, com_modname);
	else
		Cvar_Set (&scr_screenshot_name, (char *)gamescreenshotname);
	...
	{
		if (fs_modified)
			Com_Printf("Playing shareware version, with modification.\nwarning: most mods require registered version.\n");
		else
...
qbool FS_ChangeGameDirs(int numgamedirs, char gamedirs[][MAX_QPATH], qbool complain, qbool failmissing)
{
	...
	{
		if (complain)
			Com_Printf("That is too many gamedirs (%i > %i)\n", numgamedirs, MAX_GAMEDIRS);
	...
	for (i = 0;i < numgamedirs;i++)
	{
		...
		{
			if (complain)
				Com_Printf("Nasty gamedir name rejected: %s\n", gamedirs[i]);
	...
	for (i = 0;i < numgamedirs;i++)
	{
		...
		{
			if (complain)
				Com_Printf("Gamedir missing: %s%s/\n", fs_basedir, gamedirs[i]);
	...
	// write current game config before switching
	...
	// reinitialize sound system and renderer (this reloads hud/console background/etc)
...
void FS_GameDir_f (void)
{
	...
	if (Cmd_Argc() < 2)
	{
		Com_Printf("gamedirs active:");
		for (i = 0;i < fs_numgamedirs;i++)
			Com_Printf(" %s", fs_gamedirs[i]);
		Com_Printf("\n");
		return;
	}
	...
	if (numgamedirs > MAX_GAMEDIRS)
	{
		Com_Printf("Too many gamedirs (%i > %i)\n", numgamedirs, MAX_GAMEDIRS);
		return;
	}
...
================
*/
qbool FS_CheckGameDir(const char *gamedir)
{
	qbool success;
	stringlist_t list;
...
#ifdef FS_BASEDIR
	strlcpy(fs_basedir, FS_BASEDIR, sizeof(fs_basedir));
#else
...
{
	Cvar_Register (&scr_screenshot_name);
	Cvar_Register (&fs_empty_files_in_pack_mark_deletions);

	Cmd_AddCommand ("gamedir", FS_GameDir_f);
	Cmd_AddCommand ("fs_rescan", FS_Rescan_f);
...
====================
*/
static qfile_t* FS_SysOpen (const char* filepath, const char* mode, qbool nonblocking)
{
	qfile_t* file;
	int mod, opt;
	unsigned int ind;
	...
	switch (mode[0])
	{
		...
		case 'a':
			...
			break;
		default:
			Com_Printf ("FS_SysOpen(%s, %s): invalid mode\n", filepath, mode);
			return NULL;
	}
	for (ind = 1; mode[ind] != '\0'; ind++)
	{
		switch (mode[ind])
		{
			...
			case 'b':
				...
				break;
			default:
				Com_Printf ("FS_SysOpen(%s, %s): unknown character in mode (%c)\n", filepath, mode, mode[ind]);
		}
	}
	...
	file = (qfile_t *)malloc (sizeof (*file));
...
qfile_t *FS_OpenPackedFile (pack_t* pack, int pack_ind)
{
	...
	{
		Com_Printf("WARNING: can't open the compressed file %s\n"
	...
	{
		Com_Printf ("FS_OpenPackedFile: can't lseek to %s in %s (offset: %d)\n",
	...
	if (dup_handle < 0)
	{
		Com_Printf ("FS_OpenPackedFile: can't dup package's handle (pack: %s)\n", pack->filename);
		return NULL;
	}
	...
	if (pfile->flags & PACKFILE_FLAG_DEFLATED)
	{
		...
		ztk = (ztoolkit_t *)malloc (sizeof (*ztk));
		...
		{
			Com_Printf ("FS_OpenPackedFile: inflate init error (file: %s)\n", pfile->name);
			close(dup_handle);
			free(file);
			return NULL;
		}
...
static searchpath_t *FS_FindFile (const char *name, int* index, qbool quiet)
{
				...
				if (!diff)
				{
					if (fs_empty_files_in_pack_mark_deletions.integer && pak->files[middle].realsize == 0)
					{
						...
						if (!quiet && developer.integer >= 10)
							Com_Printf("FS_FindFile: %s is marked as deleted\n", name);
	...
	if (!quiet && developer.integer >= 10)
		Com_Printf("FS_FindFile: can't find %s\n", name);
...
===========
*/
qfile_t *FS_OpenReadFile (const char *filename, qbool quiet, qbool nonblocking)
{
	searchpath_t *search;
	int pack_ind;
...
qfile_t* FS_Open (const char* filepath, const char* mode, qbool quiet, qbool nonblocking)
{
	...
	if (mode[0] == 'w' || mode[0] == 'a' || strchr (mode, '+'))
	{
		...
		Q_snprintf (real_path, sizeof (real_path), "%s/%s", fs_gamedir, filepath);
...
fs_offset_t FS_Read (qfile_t* file, void* buffer, size_t buffersize)
{
	...
	while (buffersize > 0)
	{
		...
		if (ztk->in_ind == ztk->in_len)
		{
			...
			{
				Com_Printf ("FS_Read: unexpected end of file\n");
				break;
			}
		...
		if (buffersize < sizeof (file->buff) / 2)
		{
			...
			{
				Com_Printf ("FS_Read: Can't inflate file\n");
				break;
			}
...
============
*/
unsigned char *FS_LoadFile (const char *path, qbool quiet, fs_offset_t *filesizepointer)
{
	qfile_t *file;
...
============
*/
qbool FS_WriteFile (const char *filename, void *data, fs_offset_t len)
{
	qfile_t *file;
	...
	if (!file)
	{
		Com_Printf("FS_WriteFile: failed on %s\n", filename);
		return false;
	}

	Com_DPrintf("FS_WriteFile: %s\n", filename);
...
fssearch_t *FS_Search(const char *pattern, int caseinsensitive, int quiet)
{
	...
	if (i > 0)
	{
		Com_Printf("Don't use punctuation at the beginning of a search pattern!\n");
		return NULL;
	}
				...
				while (temp[0])
				{
					if (matchpattern(temp, (char *)pattern, true))
					{
						...
						if (resultlistindex == resultlist.numstrings)
						{
							...
							if (!quiet)
								Com_DPrintf("SearchPackFile: %s : %s\n", pak->filename, temp);
						}
					}
	...
	if (resultlist.numstrings)
	{
		...
		search = (fssearch_t *)malloc(sizeof(fssearch_t) + numchars + numfiles * sizeof(char *));
		memset(search, 0, sizeof(search));
...
int FS_CRCFile(const char *filename, size_t *filesizepointer)
{
	...
	if (!filename || !*filename)
		return crc;
	filedata = FS_LoadFile(filename, true, &filesize);
	if (filedata)
	{
		if (filesizepointer)
			*filesizepointer = filesize;
