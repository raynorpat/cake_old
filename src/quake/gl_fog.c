...
void Fog_Update (float density, float red, float green, float blue, float time)
{
	...
	if (time > 0) {
		...
		if (fade_done > r_refdef2.time) {
			float f;

			f = (fade_done - r_refdef2.time) / fade_time;
...
static void Fog_FogCommand_f (void)
{
	...
	case 1:
		Com_Printf("usage:\n");
		Com_Printf("   fog <density>\n");
		Com_Printf("   fog <red> <green> <blue>\n");
		Com_Printf("   fog <density> <red> <green> <blue>\n");
		Com_Printf("current values:\n");
		Com_Printf("   \"density\" is \"%f\"\n", fog_density);
		Com_Printf("   \"red\" is \"%f\"\n", fog_red);
		Com_Printf("   \"green\" is \"%f\"\n", fog_green);
		Com_Printf("   \"blue\" is \"%f\"\n", fog_blue);
		break;
	case 2:
...
{
	qglFogfv(GL_FOG_COLOR, Fog_GetColor());
	qglFogf(GL_FOG_DENSITY, Fog_GetDensity() / 64.0);
}
